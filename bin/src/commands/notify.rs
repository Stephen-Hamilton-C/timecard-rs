    ...
    fn load(path: &PathBuf) -> anyhow::Result<NotifyData> {
        if fs::exists(path).unwrap_or(false) {
            ...
            let notify: NotifyData =
                serde_json::from_str(&notify_data).context("Failed to parse notify data")?;
            Ok(notify)
        } else {
            Ok(NotifyData {
...
fn install_notify_service() -> anyhow::Result<()> {
    ...
    let service_dir = config_home.join("systemd").join("user");
    ...
    fs::write(&unit_path, unit_data).context("Failed to create systemd unit service")?;
    println!("{} {}", "CREATE".green(), timer_path.display());
    fs::write(&timer_path, SERVICE_TIMER).context("Failed to create systemd timer")?;
    ...
    println!(
        "{} systemd --user enable --now timecard-notify.timer",
        "RUN".cyan()
    );
    let status = Command::new("systemctl")
        .arg("--user")
...
fn install_notify_service() -> anyhow::Result<()> {
    eprintln!(
        "{}",
        "Notification service install is only supported on Linux systems running systemd.".yellow()
    )
}
